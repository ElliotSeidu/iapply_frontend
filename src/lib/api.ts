...
async function performRefresh(): Promise<string | null> {
  ...
  try {
    const { data } = await axios.post(`${API_BASE_URL}/account/login/refresh/`, { refresh });
...
export async function loginRequest(email: string, password: string): Promise<AuthTokens & Partial<User>> {
  const { data } = await axios.post(`${API_BASE_URL}/account/login/`, { email, password });
  return data;
}
...
export async function listApplications(): Promise<Application[]> {
  const { data } = await api.get<PaginatedResponse<Application> | Application[]>('/tracker/applications/', {
...
export async function createApplication(payload: ApplicationInput): Promise<Application> {
  const { data } = await api.post<Application>('/tracker/applications/', payload);
  return data;
}

export async function updateApplication(id: string, payload: Partial<ApplicationInput>): Promise<Application> {
  const { data } = await api.patch<Application>(`/tracker/applications/${id}/`, payload);
  return data;
}
...
export async function logStatusChange(
  ...
  await api.post(`/tracker/applications/${id}/log-status/`, { status, notes });
...
export async function listStaleApplications(): Promise<Application[]> {
  const { data } = await api.get<PaginatedResponse<Application> | Application[]>('/tracker/applications/stale/');
  return unwrapList(data);
}
...
export async function listReminders(): Promise<Reminder[]> {
  const { data } = await api.get<PaginatedResponse<Reminder> | Reminder[]>('/tracker/reminders/', {
    params: { page_size: 500 },
  });
  return unwrapList(data);
}
...
export async function updateReminder(id: string, payload: Partial<ReminderInput>): Promise<Reminder> {
  const { data } = await api.patch<Reminder>(`/tracker/reminders/${id}/`, payload);
  return data;
}
